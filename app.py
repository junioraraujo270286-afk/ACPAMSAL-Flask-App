...
app = Flask(__name__)
app.secret_key = "acpamsal_gestao_2026_secret_key"

# --- CONFIGURAÇÃO DE AMBIENTE E BANCO DE DADOS ---
BASE_DIR = os.path.abspath(os.path.dirname(__file__))

# Captura a URL do banco do Render (PostgreSQL)
DATABASE_URL = os.environ.get('DATABASE_URL')

if DATABASE_URL:
    # O SQLAlchemy exige 'postgresql://' em vez de 'postgres://' fornecido pelo Render
    if DATABASE_URL.startswith("postgres://"):
        DATABASE_URL = DATABASE_URL.replace("postgres://", "postgresql://", 1)
else:
    # Caso rode localmente sem variável de ambiente
    DATABASE_URL = 'sqlite:///' + os.path.join(BASE_DIR, 'acpamsal_gestao.db')
...
class Configuracao(db.Model):
    ...
    nome_associacao = db.Column(db.String(500), default="Associação dos Condutores Profissionais Autônomos Motofretistas, Motociclistas, Motoboys e Mototaxistas de Salinópolis - ACPAMSAL")
...
def atualizar_status_socios():
    try:
        ...
        for s in socios:
            ...
            # Regra: 3 ou mais mensalidades em atraso = Desligado
...
def login_socio():
    if request.method == 'POST':
        ...
        flash("E-mail ou Matrícula inválidos.")
...
def consulta_publica():
    ...
    resultados = Associado.query.filter(
        (Associado.status == "ATIVO"),
        (Associado.matricula == termo) | (Associado.nome.like(f"%{termo}%")) | (Associado.placa == termo)
    ).all()
...
def admin_mensalidades():
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        atualizar_status_socios()
        flash(f"Mensalidade de {mes} gerada para todos.")
    
    mensalidades = Mensalidade.query.order_by(Mensalidade.id.desc()).all()
    return render_template('admin_mensalidades.html', mensalidades=mensalidades)
...
def gerar_cracha(id):
    ...
    c.rect(0, 0, 54*mm, 86*mm, fill=1)
    c.setFillColor(colors.white)
    c.rect(2*mm, 2*mm, 50*mm, 82*mm, fill=1)
    
    # Foto do Sócio
    if socio.foto:
        img_path = os.path.join(app.config['UPLOAD_FOLDER'], socio.foto)
        if os.path.exists(img_path):
            c.drawImage(img_path, 12*mm, 45*mm, 30*mm, 30*mm)
    
    c.setFillColor(colors.black)
    c.setFont("Helvetica-Bold", 8)
    c.drawCentredString(27*mm, 38*mm, socio.nome[:22].upper())
    c.setFont("Helvetica", 7)
    c.drawCentredString(27*mm, 34*mm, f"MATRÍCULA: {socio.matricula}")
    c.drawCentredString(27*mm, 30*mm, f"PLACA: {socio.placa}")
    
    # Rodapé com nome da Associação
    c.setFillColor(colors.HexColor("#1a1a1a"))
    c.rect(2*mm, 2*mm, 50*mm, 8*mm, fill=1)
    c.setFillColor(colors.white)
    c.setFont("Helvetica-Bold", 4)
    c.drawCentredString(27*mm, 5*mm, conf.nome_associacao[:60])
